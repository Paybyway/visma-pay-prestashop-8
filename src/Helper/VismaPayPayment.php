...
class VismaPayPayment
{
    ...
    public function createPayment(\Cart $cart): ?string
    {
        ...
        try {
            ...
            return VismaPaySDK::API_URL . "/token/$response->token";
    ...
    private function saveVismaPayOrder(\Cart $cart): string
    {
        ...
        if (!\Db::getInstance()->getRow('SELECT id FROM ' . _DB_PREFIX_ . 'vismapay_order WHERE cart_id=' . (int) $cart->id)) {
            \Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'vismapay_order (`cart_id`, `order_number`, `amount`) VALUES (' . (int) $cart->id . ", '" . pSQL($orderNumber) . "', " . (float) $rawAmount . ')');
        } else {
            \Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . "vismapay_order SET order_number='" . pSQL($orderNumber) . "', amount=" . (float) $rawAmount . ' WHERE cart_id=' . (int) $cart->id);
        }
    ...
    private function getProducts(\Cart $cart): array
    {
        ...
        if ($discountAmount > 0) {
            ...
            $products[] = [
                'id' => 1,
                'title' => $this->module->getTranslator()->trans('Total discounts', [], 'Modules.Vismapay.VismaPayPayment'),
                'count' => 1,
    ...
    private function getPaymentMethod(\Cart $cart): ?array
    {
        ...
        $supportedLangauges = ['fi', 'en', 'sv', 'ru'];
